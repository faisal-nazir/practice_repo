...
public class LongestAbsoluteFilePath {
	 ...
	 * directory dir contains two sub-directories subdir1 and subdir2. subdir1
	...
	public static int lengthLongestPath_01(String input) {
		...
		for (String s : input.split("\n")) {
			...
			// check if it is file
			if (s.contains("."))
				maxLen = Math.max(maxLen, len - 1); // -1 because of last '/'
													// character
		}
		return maxLen;
	}
	...
	public static void main(String[] args) {
		String s = "dir\n\tsubdir1\n\t\tfile1.ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2\n\t\t\tfile2.ext";
		System.out.print(lengthLongestPath_02(s));
	}

	// same idea as above but using arrays instead of stack
	public static int lengthLongestPath_02(String input) {
		String[] paths = input.split("\n");
		int[] stack = new int[paths.length + 1];
		int maxLen = 0;
		for (String s : paths) {
			int lev = s.lastIndexOf("\t") + 1, curLen = stack[lev + 1] = stack[lev]
					+ s.length() - lev + 1; // minus level to remove 't's and
											// plus one to add a last '/' to
											// current length
			if (s.contains("."))
				maxLen = Math.max(maxLen, curLen - 1); // minus one to remove
														// the last '/' if this

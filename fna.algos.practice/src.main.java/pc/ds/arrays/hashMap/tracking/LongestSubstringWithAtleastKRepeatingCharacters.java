...
public class LongestSubstringWithAtleastKRepeatingCharacters {
	...
	/**
	 * ioctl super cool solution! many thanks for sharing! this problem prompts
	 * us to use two pointer method, however it's quite difficult to decide the
	 ...
	 * answer. Details: (for a fixed h)
	 * 
	 * basically, we want to find a window (i to j) which has "h" unique chars
	 * and if all h occur at least K times, we have a candidate solution [Rules
	 * for window Expansion] so expand (j++) as long as the num of unique
	 ...
	 * should start to shrink now. [Rules to window Shrink window] shrink as

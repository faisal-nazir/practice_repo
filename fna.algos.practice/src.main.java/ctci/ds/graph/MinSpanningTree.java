...
import common.utils.MinHeap;

public class MinSpanningTree {
	
	// https://github.com/mission-peace/interview/blob/master/src/com/interview/graph/PrimMST.java
	public List<Edge<String>> primsMST(Graph<String> g) {
		MinHeap<Vertex<String>> heap = new MinHeap<>();
		Map<Vertex<String>, Edge<String>> vertexToEdge = new HashMap<>();
        List<Edge<String>> result = new ArrayList<>();
		
		for(Vertex<String> v : g.getAllVertex()) {
			heap.add(v, Integer.MAX_VALUE);
		}
		
		Vertex<String> start = g.getAllVertex().iterator().next();
		heap.decrease(start, 0);
		while(!heap.isEmpty()) {
			Vertex<String> current = heap.extractMin();
			
			Edge<String> spanningTreeEdge = vertexToEdge.get(current);
...
//	        graph.addEdge(3, 1, 1);
	        ...
	        MinSpanningTree prims = new MinSpanningTree();
	        Collection<Edge<String>> edges = prims.primsMST(graph);
